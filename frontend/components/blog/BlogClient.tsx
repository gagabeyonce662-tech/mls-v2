...
export default function BlogClient({ posts, categories, fallbackImage }: BlogClientProps) {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return filtered.filter(post => post.status === "published" || post.status === "draft");
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPosts.map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}`}>
                <article className="rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow cursor-pointer" style={{ backgroundColor: colors.cards }}>
                  {/* Image */}
                  ...
                  {/* Content */}
                  <div className="p-6">
                    <h3 className="text-lg font-bold mb-3 leading-snug line-clamp-2" style={{ color: colors.heading }}>
                      {post.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {(post.embed_url || post.video_file) && (
                        <span className="text-xs px-2 py-1 rounded flex items-center gap-1" style={{ backgroundColor: colors.boarder, color: colors.body }}>
