...
export default function DesktopNav({ isScrolled }: DesktopNavProps) {
    ...
    return (
        <motion.header
            ...
            className={`hidden lg:block fixed top-0 left-0 right-0 z-50 transition-all duration-500 antialiased ${isScrolled
                ? "shadow-2xl py-1"
                : "py-3"
                }`}
        >
                        ...
                        <nav className="flex items-center space-x-6">
                            {navigation.map((item) => {
                                ...
                                return (
                                    ...
                                    >
                                        {item.name}
                                        <span className={`absolute -bottom-1.5 left-0 h-0.5 transition-all duration-300 ${isActive ? "w-full bg-[#4C7DFF]" : "w-0 group-hover:w-full bg-[#4C7DFF]/50"
                                            }`} />
                                    </Link>
                                );
                            })}
