...
export function ProvinceProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load saved province from localStorage on mount
  useEffect(() => {
    const savedProvince = localStorage.getItem('selectedProvince');
    if (savedProvince && CANADIAN_PROVINCES.some(p => p.code === savedProvince)) {
  ...
  const getProvinceCode = React.useCallback((name: string): string => {
    const province = CANADIAN_PROVINCES.find(p => p.name.toLowerCase() === name.toLowerCase());
